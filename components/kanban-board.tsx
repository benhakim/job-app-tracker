...
import { closestCorners, DndContext, DragEndEvent, DragOverlay, DragStartEvent, PointerSensor, useDraggable, useDroppable, useSensor, useSensors } from "@dnd-kit/core";
...
function DroppableColumn({
  ...
  return (
    <Card className="min-w-75 shrink-0 shadow-md p-0">
      ...
      <CardContent 
      ref={setNodeRef}
      className={`space-y-2 pt-4 bg-gray-50/50 min-h-100px rounded-b-lg
...
function SortableJobCard({job, columns}
  : {job: JobApplication, columns: Column[]}) {
    const {attributes,
       listeners, 
       setNodeRef, 
       transform, 
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  const targetColumn = sortedColumns.find((col) => 
    col._id === overId);
  const targetJob = sortedColumns.flatMap((col) => col.jobApplications || [])
  ...
  if (targetColumn) {
    ...
    const allobsInTargetOriginal = targetColumnObj.jobApplications
    .sort((a, b) => a.order - b.order) || [];
   
    const allJobsInTargetFiltered = allobsInTargetOriginal
    .filter((j) => j._id !== activeId) || [];
    
    const targetIndexInFiltered = allJobsInTargetFiltered
    .findIndex(j => j._id === overId);

    if (targetIndexInFiltered !== -1) {
      if (sourceColumn._id === targetColumnId) {
        newOrder = sourceIndex < targetIndexInFiltered ? targetIndexInFiltered : targetIndexInFiltered + 1;
      } else {
